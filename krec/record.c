/*
This is a modified record utility for the Kinect. The original code is from libfreenect's fakenect-record. 
The dump files are no longer saved, an option has been added to set a maximum record time 
and a motion detection trigger that can enable recording.
...
int record = 1;
int cnt = 0;
uint16_t ref_frame[640*480];
uint16_t cmp_frame[640*480];

void signal_cleanup(int num);

void to_cmp_frame(void *data, uint16_t *frame)
{
	uint8_t *data_ptr;
	int i;
	data_ptr = (uint8_t *)data;

	for(i = 0; i!= FREENECT_FRAME_W * FREENECT_FRAME_H; ++i) {
		frame[i] = ((uint16_t)(data_ptr[i*2+1] << 8) & (0xFF00));
		frame[i] += data_ptr[i*2];
	}
}

uint32_t compare_frames()
{
	uint64_t dif = 0;
	int i;

	for(i=0;i!=FREENECT_FRAME_W*FREENECT_FRAME_H; ++i) {
		dif += abs(ref_frame[i] - cmp_frame[i] );
	}

	return dif / (FREENECT_FRAME_W*FREENECT_FRAME_H);
}
...
void depth_cb(freenect_device *dev, void *depth, uint32_t timestamp)
{
	...
	if(record==1) {
		...
		if (cnt++ == 5) {
			to_cmp_frame(depth, ref_frame);
		} else if (cnt > 5) {
			to_cmp_frame(depth, cmp_frame);
			cmp_val = compare_frames();
			printf("%d\n", cmp_val);
			if(cmp_val > trigger) {
				if(rectime!=0)
					endtime = get_time() + rectime;
				record = 1;
			}
...
int main(int argc, char **argv)
{
	...
	while (c < argc) {
		if (strcmp(argv[c],"-ffmpeg")==0)
			...
			if (++c < argc)
				ffmpeg_opts = argv[c];
		} else if(strcmp(argv[c],"-rectime")==0) {
			if (++c < argc)
				//endtime = get_time() + atof(argv[c]);
				rectime = atoi(argv[c]);
		} else if(strcmp(argv[c],"-trigger")==0) {
			record = 0;
			if (++c < argc)
				trigger = atoi(argv[c]);
